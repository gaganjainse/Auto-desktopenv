...
This folder is the single source of truth for the **Sesha** ecosystem — the production-grade,
AI-first, hands-off desktop built on top of your `Auto-desktopenv` fork. Every document here was
independently verified against the live repo on 2026-08-09; it supersedes the two prior AI audits
(`uploads/Auto-desktopenv-audit.md` and the 63-page `Auto-desktopenv_Master_Plan_90p.pdf`), both of
...
| 06 | `06_SESHA_AGENT.md` | The Sesha agent: Newelle + Ollama + MCP, local-first | Building the assistant |
...
- `docs/SESHA/checklist.md` — one-line-per-task checklist you tick off.
...
From your chats, GitHub profile (`AI/LLM Engineer, VIT Vellore 2025, author of NexusAOS`), and
repos (NexusAOS — governance-first, event-sourced, append-only audit trail; SeshaOS — local-first
...
2. **Governance & auditability.** Borrow NexusAOS's append-only event log: every autonomous action
   Sesha takes is recorded, replayable, and reversible. Policy gates destructive actions.
...
7. **Indian-rooted identity.** The names (Sesha, Vyākṛti, Nexus) and the Sanskrit/Indic framing are
...
**Sesha** is the local AI layer that sits between you and your CachyOS/Hyprland machine: it watches
...
all while writing every action to a NexusAOS-style audit log so nothing it does is invisible or
irreversible. Over time it becomes the desktop manifestation of your NexusAOS/SeshaOS thesis:
