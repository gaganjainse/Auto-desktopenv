...
```
You are working on the "Sesha" ecosystem — Gagan's local-first, AI-first CachyOS/Hyprland desktop
fork of end-4/dots-hyprland. Repo: /home/user/Auto-desktopenv (a git repo).
...
4. One logical change per turn. Update docs/SESHA/checklist.md.
...
04_DEVICE_PROFILE.md), mkinitcpio.fragment, kernel-cmdline.txt, sysctl/99-sesha.conf,
...
HX". Every file must have a comment pointing back to docs/SESHA/04_DEVICE_PROFILE.md. Provide
...
```
Implement smart-organizer v2 per docs/SESHA/05_SMART_ORGANIZER_V2.md:
...
docs/SESHA/06_SESHA_AGENT.md section 5; (c) create smart_organizer.py with organize/last_moves/
...
```
Create tools/sesha/sesha_audit.py: an append-only JSONL + SQLite writer with a chained SHA-256 hash
...
refused. Add `sesha log` and `sesha undo` CLI subcommands. Include tests that tampering breaks the
chain.
```

### 6.3 Quickshell overlay
```
Create dots/.config/quickshell/ii/sesha/SeshaOverlay.qml: a small Material-You-colored pill (reuse
...
hardware/OS facts in docs/SESHA/00_INDEX.md? (2) does it introduce undefined vars, missing quoting,
...
re-running completed steps. If it's a known issue from docs/SESHA/01_AUDIT.md, cite its ID.
```
...
```
I want to automate: <describe>. Following docs/SESHA/07_AUTOMATIONS.md conventions, produce a
...
```
Read docs/SESHA/checklist.md and git log since last week. Summarize what's done, what's next from
...
- Keep `docs/SESHA/` as the spec of record; if reality changes, update the doc in the same commit.
