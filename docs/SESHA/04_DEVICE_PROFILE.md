...
-- on battery, Sesha flips blur size/passes down via a power profile rule
```
**On battery**, reduce blur passes and disable window shadows (the iGPU draws them); this is a real
battery win and is exactly the kind of thing Sesha automates.
...
4. Emit a desktop notification on mode change and write to the Sesha audit log.
