...
# See docs/SESHA/02_ROADMAP.md (Phase 2) and 01_AUDIT.md (HIGH-06).
...
undo_ai_stack() {
  ...
  if command_exists systemctl && [[ -n "${DBUS_SESSION_BUS_ADDRESS:-}" ]]; then
    for u in sesha-system-control-mcp.service sesha-smart-organizer-mcp.service \
             sesha-hyprland-control-mcp.service; do
  ...
  v rm -f "${XDG_BIN_HOME:-$HOME/.local/bin}"/sesha-*-mcp
  v rm -f "${XDG_BIN_HOME:-$HOME/.local/bin}"/sm-watcher

  # Remove the Sesha venv (state, not config)
