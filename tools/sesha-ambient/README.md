...
- Part of: [Sesha](https://github.com/gaganjainse/Auto-desktopenv)
...
3. **Warmth, not nagging.** At a natural pause (idle 45s–15m, not busy, work
   hours), Sesha makes **one** small optional offer (break, organize downloads,
...
src/sesha_ambient/
  ...
  cli.py          # `sesha-ambient tick|offer` (real system probing)
...
uv run sesha-ambient --dry-run tick
# Check whether an offer is appropriate (for an overlay/hotkey):
uv run sesha-ambient offer
```

## How it integrates

- The **timer** fires 3 min after the graphical session starts and every 4 h, with
  catch-up + jitter; it calls `sesha-ambient tick`.
- The **Quickshell overlay** calls `sesha-ambient offer` opportunistically (e.g. on
...
See `docs/SESHA/AMBIENT_DESIGN.md` for the full rationale and the proactivity model.
